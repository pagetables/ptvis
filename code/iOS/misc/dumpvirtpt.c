...
static inline void *getpmap(void **x)
{
    x = *(x+3); //proc -> task

    x = *(x+6); //task -> vmap
    x = *(x+8+3); //vmap -> pmap;
    x = *(x+1); //second entry is the physical base (first was virtual)
    return x;
}
...
void get_regs(void *proc, void **destp, void *ret) {
    ...
    //
    void (*all_lock)();
    all_lock  = slide + 0x801D7300|1;
    all_lock();
    
    void (*all_unlock)();
    all_unlock = slide + 0x801D7314|1;
    ...
    //scan all the way forward
    while (ptr != NULL)
    {
        // proc to task
        pid = ptr[2];
        IOLog("pid f = %d proc = %p\n", pid, ptr);
        ptr = (uint32_t)ptr[1];
    }

    ptr = proc;
    //scan all the way back now
    while (ptr != NULL)
    {
        // proc to task
        pid = ptr[2];
        IOLog("pid b = %d proc = %p physbase = %p\n", pid, ptr, getpmap(ptr));
        ptr = (uint32_t)ptr[0];
    }
    ...
    // task to vm_map
    ptr = (uint32_t)*(ptr+6);
    // vm_map to pmap virtual address
    ptr = (uint32_t)*(ptr+11);
    ptr = (ptr + 2);
    
    IOLog("Dumping 32 words from virt pmap %x (proc @ %p)\n", ptr, proc);
    for(i = 0; i < 32; i++)
    {
        IOLog("%d: %08x\n", i, ptr[i]);
    }
